...
Timestamp: ${new Date().toLocaleString()}`;
  ...
  return (
    <>
      {/* Chatbot Window */}
      <div className={`gbot-chat-window ${isOpen ? 'gbot-open' : 'gbot-closed'}`}>
        <div className="gbot-chat-header">Complaint Assistant</div>
        <div className="gbot-chat-messages">
          {messages.map((msg, index) => (
            <div key={index} className={`gbot-message gbot-${msg.sender}`}>
              {msg.text}
              {msg.options && (
                <div className="gbot-options-container">
                  {msg.options.map(opt => (
                    <button key={opt} onClick={() => handleOptionSelect(opt)} className="gbot-option-button">
                      {opt}
                    </button>
                  ))}
                </div>
        ...
        {step === 'description' && (
          <form className="gbot-chat-input-container" onSubmit={handleDescriptionSubmit}>
            <input
              type="text"
              className="gbot-chat-input"
              placeholder="Type your complaint..."
              value={inputValue}
              onChange={(e) => setInputValue(e.target.value)}
              autoFocus
            />
            <button type="submit" className="gbot-send-button">
      ...
      {/* Chatbot Toggle Icon */}
      <div className={`gbot-chatbot-icon ${isOpen ? 'gbot-open' : ''}`} onClick={() => setIsOpen(!isOpen)}>
        {/* Chat Icon */}
        <svg className="gbot-chat-bubble-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path>
        </svg>
        {/* Close Icon (X) */}
        <svg className="gbot-close-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
